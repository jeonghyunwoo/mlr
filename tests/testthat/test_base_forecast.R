...
test_that("forecast learning", {
  ...
  pred = predict(mod, fcregr.task)
  p = performance(pred)
  expect_true(!is.na(p))
  # with newdata xts
  pred = predict(mod, newdata = fcregr.xts)
  p = performance(pred)
  expect_true(!is.na(p))
  # resample
  r = holdout(lrn, fcregr.task)
  expect_true(!is.na(r$aggr))
  # Learning with Lambert W
  lrn = makePreprocWrapperLambert(lrn, target.proc = TRUE, type = "h", verbose = FALSE)
  fcregr.xts2 = fcregr.xts
  fcregr.task2 = makeForecastRegrTask("fcregr", data = fcregr.xts2, target = fcregr.target, check.data = FALSE)
  mod = train(lrn, fcregr.task2)
  pred = predict(mod, fcregr.task2)
  p = performance(pred)
  expect_true(!is.na(p))
